...
{
    public class S4AnnotationClientImpl : S4AbstractClientImpl, IS4AnnotationClientImpl
    {
        /// <summary>
        /// Construct a <code>IS4AnnotationClientImpl</code> accessing a specific processing pipeline on the s4.ontotext.com platform using the given credentials.
        /// </summary>
        ...
        public S4AnnotationClientImpl(ServiceDescriptor item, String apiKey, String keySecret) : base(item, apiKey, keySecret)
        {
        }
        ...
        /// <param name="keySecret">corresponding password</param>
        public S4AnnotationClientImpl(Uri endpoint, String apiKey, String keySecret) : base(endpoint, apiKey, keySecret)
        {
        }
        ...
        /// <returns>an {@link AnnotatedDocument} containing the original content as well as the annotations produced</returns>
        public AnnotatedDocument annotateDocument(FileStream documentContent, String documentEncoding, SupportedMimeType documentMimeType)
        {
        ...
        public Stream annotateDocumentAsStream(Uri documentUrl, SupportedMimeType documentMimeType, ResponseFormat serializationFormat)
        {
            ServiceRequest rq = new ServiceRequest(documentUrl, documentMimeType);
            try
            {
        ...
        /// <returns>Аn {@link InputStream} for the serialization of the annotated document in the specified format</returns>
        private Stream processRequestForStream(ServiceRequest rq, ResponseFormat serializationFormat)
        {
            try
            {
                return client.requestForStream("", "POST", rq, constructHeaders(serializationFormat));
        ...
        /// <param name="requestCompression">whether to allow GZIP compression for large documents</param>
        /// <returns>Аn {@link AnnotatedDocument} containing the original content as well as the annotations produced</returns>
        private AnnotatedDocument processRequest(ServiceRequest rq)
        {
            try
            {
                return client.request("", "POST", new AnnotatedDocument(), rq, constructHeaders(ResponseFormat.GATE_JSON));
            }
            catch (HttpClientException e)
            {
                String msg = handleErrors(e);
